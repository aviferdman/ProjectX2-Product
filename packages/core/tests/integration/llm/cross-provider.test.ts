...
describe('Cross-provider: text generation consistency', () => {
  it('should return content, finishReason, and tokenUsage with same structure from both providers', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(openaiChatResponse({ content: 'Hello world', promptTokens: 8, completionTokens: 3 })),
    );
    const openaiResult = await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE);

    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(anthropicMessageResponse({ content: 'Hello world', inputTokens: 8, outputTokens: 3 })),
    );
    const anthropicResult = await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE);
    ...
    expect(openaiResult.tokenUsage).toEqual({ promptTokens: 8, completionTokens: 3, totalTokens: 11 });
    expect(anthropicResult.tokenUsage).toEqual({ promptTokens: 8, completionTokens: 3, totalTokens: 11 });
  });

  it('should handle empty content response from both providers', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(openaiChatResponse({ content: '' })),
    );
    const openaiResult = await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE);

    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(anthropicMessageResponse({ content: '' })),
    );
    const anthropicResult = await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE);
  ...
  it('should handle multi-turn conversations through both providers', async () => {
    ...
    const fetchMockOpenAI = vi.fn().mockResolvedValue(
      jsonResponse(openaiChatResponse({ content: '3 + 3 = 6', promptTokens: 30, completionTokens: 5 })),
    );
    ...
    const fetchMockAnthropic = vi.fn().mockResolvedValue(
      jsonResponse(anthropicMessageResponse({ content: '3 + 3 = 6', inputTokens: 30, outputTokens: 5 })),
    );
  ...
  it('should pass temperature and maxTokens to both providers', async () => {
    const fetchMockOpenAI = vi.fn().mockResolvedValue(
      jsonResponse(openaiChatResponse({ content: 'response' })),
    );
    globalThis.fetch = fetchMockOpenAI;
    await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE, { temperature: 0.7, maxTokens: 500 });

    const fetchMockAnthropic = vi.fn().mockResolvedValue(
      jsonResponse(anthropicMessageResponse({ content: 'response' })),
    );
    globalThis.fetch = fetchMockAnthropic;
    await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE, { temperature: 0.7, maxTokens: 500 });
  ...
  it('should pass stop sequences using provider-specific field names', async () => {
    ...
    globalThis.fetch = fetchMockOpenAI;
    await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE, { stopSequences: ['END', 'STOP'] });

    const fetchMockAnthropic = vi.fn().mockResolvedValue(
      jsonResponse(anthropicMessageResponse({ content: 'response' })),
    );
    globalThis.fetch = fetchMockAnthropic;
    await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE, { stopSequences: ['END', 'STOP'] });
...
describe('Cross-provider: error handling consistency', () => {
  it('should throw LLMAuthenticationError for 401 from both providers', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(openaiErrorResponse({ message: 'Invalid API key' }), 401),
    );
    await expect(new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE)).rejects.toThrow(LLMAuthenticationError);

    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(anthropicErrorResponse({ message: 'Invalid API key' }), 401),
    );
    await expect(new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE)).rejects.toThrow(LLMAuthenticationError);
  });
  ...
  it('should throw LLMRateLimitError for 429 with retry-after from both providers', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(anthropicErrorResponse({ message: 'Rate limited' }), 429, { 'retry-after': '10' }),
    );
  ...
  it('should throw LLMContextLengthError for 400 context overflow from both providers', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(openaiErrorResponse({
        message: 'This model maximum context length is 128000 tokens. However, you requested 130000 tokens.',
    ...
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(anthropicErrorResponse({
  ...
  it('should throw LLMProviderError for 500/502/503 from both providers', async () => {
    for (const status of [500, 502, 503]) {
      globalThis.fetch = vi.fn().mockResolvedValue(
        jsonResponse(openaiErrorResponse({ message: 'Server error' }), status),
      );
      await expect(new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE)).rejects.toThrow(LLMProviderError);

      globalThis.fetch = vi.fn().mockResolvedValue(
        jsonResponse(anthropicErrorResponse({ message: 'Server error' }), status),
      );
      await expect(new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE)).rejects.toThrow(LLMProviderError);
    }
  });
  ...
  it('should handle AbortError from both providers', async () => {
    ...
    const openaiErr = await new OpenAIProvider(openaiConfig())
      .generateText(SIMPLE_USER_MESSAGE).catch((e: unknown) => e as LLMProviderError);
...
describe('Cross-provider: streaming consistency', () => {
  it('should produce identical final content from streaming for both providers', async () => {
    ...
    const openaiResult = await (await new OpenAIProvider(openaiConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();
    ...
    const anthropicResult = await (await new AnthropicProvider(anthropicConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();
  ...
  it('should yield chunks incrementally from both providers', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(sseResponse(openaiSSEStream([
      ...
      { content: '', finishReason: 'stop', usage: { prompt_tokens: 3, completion_tokens: 3, total_tokens: 6 } },
    ])));
    const openaiChunks: LLMStreamChunk[] = [];
    for await (const chunk of await new OpenAIProvider(openaiConfig()).generateStream(SIMPLE_USER_MESSAGE)) {
      openaiChunks.push(chunk);
    }

    const events = anthropicStreamEvents(['chunk1', 'chunk2', 'chunk3'], { inputTokens: 3, outputTokens: 3 });
    globalThis.fetch = vi.fn().mockResolvedValue(sseResponse(anthropicSSEText(events)));
    const anthropicChunks: LLMStreamChunk[] = [];
    for await (const chunk of await new AnthropicProvider(anthropicConfig()).generateStream(SIMPLE_USER_MESSAGE)) {
  ...
  it('should handle 5-byte chunked SSE delivery from both providers', async () => {
    const openaiSSE = openaiSSEStream([
      { content: 'Hello' },
      { content: '', finishReason: 'stop', usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 } },
    ]);
    globalThis.fetch = vi.fn().mockResolvedValue(chunkedSSEResponse(openaiSSE, 5));
    const openaiResult = await (await new OpenAIProvider(openaiConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();

    const anthropicSSE = anthropicSSEText(anthropicStreamEvents(['Hello'], { inputTokens: 1, outputTokens: 1 }));
...
describe('Cross-provider: identity and configuration', () => {
  ...
  it('should reject empty API keys from both providers', () => {
    ...
    expect(() => new AnthropicProvider(anthropicConfig({ apiKey: '' }))).toThrow(LLMAuthenticationError);
  });
  ...
  it('should use correct auth header format for each provider', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(openaiChatResponse({ content: 'ok' })));
    globalThis.fetch = fetchMock;
    await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE);
    const openaiHeaders = (fetchMock.mock.calls[0] as [string, RequestInit])[1].headers as Record<string, string>;
    expect(openaiHeaders['Authorization']).toBe('Bearer sk-test-key-1234567890abcdef');

    const fetchMock2 = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ content: 'ok' })));
    globalThis.fetch = fetchMock2;
    await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE);
    const anthropicHeaders = (fetchMock2.mock.calls[0] as [string, RequestInit])[1].headers as Record<string, string>;
  ...
  it('should support custom base URLs for both providers', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(openaiChatResponse({ content: 'ok' })));
    globalThis.fetch = fetchMock;
    await new OpenAIProvider(openaiConfig({ baseUrl: 'https://openai-proxy.example.com/v1' })).generateText(SIMPLE_USER_MESSAGE);
    expect((fetchMock.mock.calls[0] as [string])[0]).toContain('openai-proxy.example.com');

    const fetchMock2 = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ content: 'ok' })));
    globalThis.fetch = fetchMock2;
    await new AnthropicProvider(anthropicConfig({ baseUrl: 'https://anthropic-proxy.example.com' })).generateText(SIMPLE_USER_MESSAGE);
...
describe('Cross-provider: provider-specific features', () => {
  it('should handle Anthropic system message extraction', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ content: 'response' })));
    globalThis.fetch = fetchMock;
    await new AnthropicProvider(anthropicConfig()).generateText([
      systemMessage('You are helpful'), systemMessage('You are concise'), userMessage('Hi'),
    ]);
  ...
  it('should handle OpenAI tool role mapping', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(openaiChatResponse({ content: '42' })));
    globalThis.fetch = fetchMock;
    await new OpenAIProvider(openaiConfig()).generateText([userMessage('Calc'), toolMessage('42', 'calc')]);
    const body = JSON.parse((fetchMock.mock.calls[0] as [string, RequestInit])[1].body as string) as Record<string, unknown>;
  ...
  it('should handle Anthropic tool role mapping to user', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ content: '42' })));
    globalThis.fetch = fetchMock;
    await new AnthropicProvider(anthropicConfig()).generateText([userMessage('Calc'), toolMessage('42', 'calc')]);
    const body = JSON.parse((fetchMock.mock.calls[0] as [string, RequestInit])[1].body as string) as Record<string, unknown>;
  ...
  it('should handle Anthropic 529 overloaded error', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse(anthropicErrorResponse({ message: 'Overloaded', type: 'overloaded_error' }), 529),
    );
  ...
  it('should set Anthropic default max_tokens to 4096', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ content: 'ok' })));
    globalThis.fetch = fetchMock;
    await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE);
    const body = JSON.parse((fetchMock.mock.calls[0] as [string, RequestInit])[1].body as string) as Record<string, unknown>;
  ...
  it('should set OpenAI stream_options in streaming requests', async () => {
    const fetchMock = vi.fn().mockResolvedValue(sseResponse(openaiSSEStream([
      { content: 'Hi', finishReason: 'stop', usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 } },
    ])));
    globalThis.fetch = fetchMock;
...
describe('Cross-provider: finish reason normalization', () => {
  it('should normalize stop from OpenAI and end_turn from Anthropic to stop', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse(openaiChatResponse({ finishReason: 'stop' })));
    expect((await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE)).finishReason).toBe('stop');

    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ stopReason: 'end_turn' })));
    expect((await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE)).finishReason).toBe('stop');
  });

  it('should normalize length from OpenAI and max_tokens from Anthropic to length', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse(openaiChatResponse({ finishReason: 'length' })));
    expect((await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE)).finishReason).toBe('length');

    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse(anthropicMessageResponse({ stopReason: 'max_tokens' })));
    expect((await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE)).finishReason).toBe('length');
  });
  ...
  it('should handle null finish reasons from both providers as unknown', async () => {
    ...
    expect((await new OpenAIProvider(openaiConfig()).generateText(SIMPLE_USER_MESSAGE)).finishReason).toBe('unknown');

    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse({
      content: [{ type: 'text', text: 'ok' }], stop_reason: null,
      usage: { input_tokens: 1, output_tokens: 1 },
    }));
    expect((await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE)).finishReason).toBe('unknown');
  });
});
...
describe('Cross-provider: realistic API response shapes', () => {
  it('should handle OpenAI response with all standard fields', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse({
      id: 'chatcmpl-ABC123xyz', object: 'chat.completion', created: 1711000000,
      model: 'gpt-4o-2024-11-20',
      choices: [{ message: { content: 'Full response', role: 'assistant' }, finish_reason: 'stop', index: 0, logprobs: null }],
      usage: { prompt_tokens: 25, completion_tokens: 10, total_tokens: 35, completion_tokens_details: { reasoning_tokens: 0 } },
  ...
  it('should handle Anthropic response with cache token fields', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse({
      id: 'msg_01ABC123xyz', type: 'message', role: 'assistant',
  ...
  it('should handle Anthropic response with mixed text and tool_use blocks', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonResponse({
      ...
      stop_reason: 'tool_use', usage: { input_tokens: 20, output_tokens: 25 },
    }));
    const result = await new AnthropicProvider(anthropicConfig()).generateText(SIMPLE_USER_MESSAGE);
...
describe('Cross-provider: streaming finish reason normalization', () => {
  it('should normalize streaming stop from OpenAI', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(sseResponse(openaiSSEStream([
      { content: 'ok', finishReason: 'stop', usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 } },
    ])));
    const result = await (await new OpenAIProvider(openaiConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();
    expect(result.finishReason).toBe('stop');
  });

  it('should normalize streaming end_turn from Anthropic to stop', async () => {
    const events = anthropicStreamEvents(['ok'], { inputTokens: 1, outputTokens: 1, stopReason: 'end_turn' });
    globalThis.fetch = vi.fn().mockResolvedValue(sseResponse(anthropicSSEText(events)));
    const result = await (await new AnthropicProvider(anthropicConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();
    expect(result.finishReason).toBe('stop');
  });

  it('should normalize streaming length from OpenAI', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(sseResponse(openaiSSEStream([
      { content: 'truncated', finishReason: 'length', usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 } },
    ])));
    const result = await (await new OpenAIProvider(openaiConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();
  ...
  it('should normalize streaming max_tokens from Anthropic to length', async () => {
    const events = anthropicStreamEvents(['truncated'], { inputTokens: 1, outputTokens: 1, stopReason: 'max_tokens' });
    globalThis.fetch = vi.fn().mockResolvedValue(sseResponse(anthropicSSEText(events)));
    const result = await (await new AnthropicProvider(anthropicConfig()).generateStream(SIMPLE_USER_MESSAGE)).toResponse();
    expect(result.finishReason).toBe('length');
  });
