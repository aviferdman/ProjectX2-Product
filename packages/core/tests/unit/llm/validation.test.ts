...
describe("LLMRequestOptionsSchema", () => {
  ...
  it("should reject temperature below 0", () => {
    expect(() => LLMRequestOptionsSchema.parse({ temperature: -0.1 })).toThrow(
      ZodError,
    );
  });

  it("should reject temperature above 2", () => {
    expect(() => LLMRequestOptionsSchema.parse({ temperature: 2.1 })).toThrow(
      ZodError,
    );
  });
  ...
  it("should reject non-integer maxTokens", () => {
    expect(() => LLMRequestOptionsSchema.parse({ maxTokens: 1024.5 })).toThrow(
      ZodError,
    );
  });

  it("should reject non-positive maxTokens", () => {
    expect(() => LLMRequestOptionsSchema.parse({ maxTokens: 0 })).toThrow(
      ZodError,
    );
    expect(() => LLMRequestOptionsSchema.parse({ maxTokens: -1 })).toThrow(
      ZodError,
    );
  });
  ...
  it("should reject unknown properties in strict mode", () => {
    expect(() =>
      LLMRequestOptionsSchema.parse({ temperature: 0.5, unknownField: true }),
    ).toThrow(ZodError);
  });
});
...
describe("LLMMessageSchema", () => {
  ...
  it("should reject empty content", () => {
    expect(() =>
      LLMMessageSchema.parse({ role: LLMRole.USER, content: "" }),
    ).toThrow(ZodError);
  });

  it("should reject invalid role", () => {
    expect(() =>
      LLMMessageSchema.parse({ role: "invalid", content: "test" }),
    ).toThrow(ZodError);
  });
});
...
describe("LLMMessagesSchema", () => {
  ...
  it("should reject messages with only assistant role", () => {
    expect(() =>
      LLMMessagesSchema.parse([
        { role: LLMRole.ASSISTANT, content: "response" },
      ]),
    ).toThrow(ZodError);
  });
  ...
  it("should reject messages with only tool role", () => {
    expect(() =>
      LLMMessagesSchema.parse([
        { role: LLMRole.TOOL, content: "result", name: "tool" },
      ]),
    ).toThrow(ZodError);
  });
...
describe("LLMProviderConfigSchema", () => {
  ...
  it("should reject empty provider name", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ provider: "", modelId: "gpt-4o" }),
    ).toThrow(ZodError);
  });

  it("should reject provider name starting with uppercase", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ provider: "OpenAI", modelId: "gpt-4o" }),
    ).toThrow(ZodError);
  });

  it("should reject provider name starting with digit", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ provider: "1openai", modelId: "gpt-4o" }),
    ).toThrow(ZodError);
  });
  ...
  it("should reject empty modelId", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ provider: "openai", modelId: "" }),
    ).toThrow(ZodError);
  });
  ...
  it("should reject negative maxRetries", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ ...validConfig, maxRetries: -1 }),
    ).toThrow(ZodError);
  });
  ...
  it("should reject maxRetries above 10", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ ...validConfig, maxRetries: 11 }),
    ).toThrow(ZodError);
  });

  it("should reject timeout below 1000ms", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ ...validConfig, timeout: 500 }),
    ).toThrow(ZodError);
  });

  it("should reject timeout above 300000ms", () => {
    expect(() =>
      LLMProviderConfigSchema.parse({ ...validConfig, timeout: 300_001 }),
    ).toThrow(ZodError);
  });
...
describe("LLMModelInfoSchema", () => {
  ...
  it("should accept model info without cost fields", () => {
    const { costPer1kInputTokens, costPer1kOutputTokens, ...noCost } =
      validModelInfo;
  ...
  it("should reject negative context tokens", () => {
    expect(() =>
      LLMModelInfoSchema.parse({ ...validModelInfo, maxContextTokens: -1 }),
    ).toThrow(ZodError);
  });

  it("should reject empty modelId", () => {
    expect(() =>
      LLMModelInfoSchema.parse({ ...validModelInfo, modelId: "" }),
    ).toThrow(ZodError);
  });
