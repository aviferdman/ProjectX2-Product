...
export interface OAuthProviderCardProps extends Omit<
...
export const OAuthProviderCard = forwardRef<HTMLDivElement, OAuthProviderCardProps>(
  function OAuthProviderCard({ connection, onConnect, onDisconnect, className, ...props }, ref) {
    const isInProgress =
      connection.status === 'connecting' || connection.status === 'disconnecting';
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          {connection.status === 'error' && connection.error && (
            <span className="text-xs text-rose-400 truncate">{connection.error}</span>
          )}
