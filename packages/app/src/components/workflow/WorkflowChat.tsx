  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-[var(--cs-border-subtle)]">
        <div className="relative rounded-xl border border-[var(--cs-border-default)] bg-[var(--cs-surface-card)] focus-within:border-violet-500/40 transition-colors">
          <textarea
            ref={inputRef}
            value={input}
...
function QuickAction({ label, onClick }: { label: string; onClick: () => void }): React.JSX.Element {
  return (
    <button
      onClick={onClick}
      className="px-2.5 py-1 rounded-lg bg-[var(--cs-surface-card)] border border-[var(--cs-border-default)] text-xs text-[var(--cs-text-secondary)] hover:text-[var(--cs-text-primary)] hover:bg-violet-500/10 hover:border-violet-500/30 transition-all focus-ring"
    >
      {label}
    </button>
  );
