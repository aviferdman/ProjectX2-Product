    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-white truncate">Workflow Assistant</h3>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scrollbar-thin scrollbar-thumb-white/10">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500/20 to-purple-500/20 border border-violet-500/20 flex items-center justify-center mb-4">
            ...
            <p className="text-sm text-slate-400 mb-2">Describe your initiative</p>
      ...
      <div className="p-3 border-t border-white/5">
        <div className="relative rounded-xl border border-white/10 bg-white/[0.03] focus-within:border-violet-500/40 transition-colors">
          <textarea
            ref={inputRef}
            value={input}
            ...
            rows={1}
            className="w-full bg-transparent text-sm text-white placeholder:text-slate-500 px-3 pt-2.5 pb-9 resize-none outline-none min-h-[40px] max-h-[120px]"
          />
          <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-2 py-1.5">
            <div className="flex items-center gap-1">
              <button className="p-1 rounded text-slate-600 hover:text-slate-400 transition-colors" title="Attach">
            ...
            <button
              ...
              className="p-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:bg-violet-600/30 disabled:cursor-not-allowed text-white transition-all"
            >
...
function MessageBubble({ message }: { message: ChatMessage }): React.JSX.Element {
  if (message.role === 'system') {
    return (
      <div className="flex items-center gap-2 py-1">
        ...
        <span className="text-[11px] text-slate-500 px-2">{message.content}</span>
        <div className="h-px flex-1 bg-white/5" />
      </div>
    );
  }
  ...
  if (message.role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] px-3.5 py-2.5 rounded-2xl rounded-tr-md bg-violet-600/90 text-sm text-white leading-relaxed">
          {message.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="max-w-[85%] text-sm text-slate-300 leading-relaxed">
...
function MarkdownLite({ text }: { text: string }): React.JSX.Element {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('•') || line.startsWith('-')) {
      ...
      elements.push(
        <div key={i} className="flex gap-2 ml-1 mt-0.5">
          <span className="text-violet-400 font-mono text-xs mt-0.5 w-4">{num}.</span>
...
function QuickAction({ label, onClick }: { label: string; onClick: () => void }): React.JSX.Element {
  return (
    <button
      onClick={onClick}
      className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-400 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all"
    >
      {label}
    </button>
  );
