          ...
          {workflow.status === 'completed' ? (
            <>
              <QuickAction
                ...
                onClick={() => onSendMessage('Summarize the workflow results')}
              />
              <QuickAction
                label="Export as report"
                onClick={() => onSendMessage('Export the results as a structured report')}
              />
            </>
            ...
            <>
              <QuickAction
                ...
                onClick={() => onSendMessage('Explain what failed and suggest fixes')}
              />
              <QuickAction
                label="Retry failed tasks"
                onClick={() => onSendMessage('Retry the failed tasks')}
              />
            </>
          ) : (
            <>
              {workflow.agents.length > 0 && (
                <QuickAction
                  ...
                  onClick={() =>
                    onSendMessage(
                      `Explain what the ${workflow.agents[0]?.role ?? 'first agent'} does in this workflow`,
                    )
                  }
                />
              )}
        ...
        <div className="relative rounded-xl border border-[var(--cs-border-default)] bg-[var(--cs-surface-card)] focus-within:border-indigo-500/40 transition-colors">
          ...
          <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-2 py-1.5">
            <div className="flex items-center gap-1">
              <button
                className="p-1 rounded text-[var(--cs-text-tertiary)] hover:text-[var(--cs-text-secondary)] transition-colors"
                title="Attach"
              >
                <svg
                  width="14"
