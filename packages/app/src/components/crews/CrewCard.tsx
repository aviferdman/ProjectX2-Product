    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center mb-4">
          <div className="flex -space-x-2">
            {visibleAgents.map((agent, i) => (
              <div
                key={agent.id}
                className="w-7 h-7 rounded-full border-2 border-[var(--cs-surface-card)] flex items-center justify-center text-white"
        ...
        <div className="flex items-center justify-between text-[11px] text-[var(--cs-text-tertiary)]">
          <span>
            {crew.agents.length} agent{crew.agents.length !== 1 ? 's' : ''} &middot;{' '}
            {crew.workflowIds.length} workflow{crew.workflowIds.length !== 1 ? 's' : ''}
          </span>
